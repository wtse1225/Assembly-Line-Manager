...
namespace sdds {
	...
	void CustomerOrder::fillItem(Station& station, std::ostream& os)
	{
		...
		for (size_t i = 0; i < m_cntItem && haveIt == false; i++)
		{
			...
			if (haveIt)
			{
				...
				{
					// To accommodate the case where there are multiple out of stock orders for the same item from the same customer
	...
	void CustomerOrder::display(std::ostream& os) const
	{
		if (os)
		{
			...
			for (size_t i = 0; i < m_cntItem; i++)
			{				
				os << "[" << setfill('0') << setw(6) << right << m_lstItem[i]->m_serialNumber << "] " << setfill(' ')
